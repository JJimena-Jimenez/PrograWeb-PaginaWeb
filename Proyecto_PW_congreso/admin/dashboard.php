...
/* ============================================================
   ...
   FUNCIÓN: Panel de administración del congreso
   ============================================================
   ACCESO: Solo para admins logueados (requireAdmin() verifica sesión)
   URL: /admin/dashboard.php

   SECCIONES (via ?s=...):
   - inicio      → Cards: total ponentes, participantes, ingresos, registros del mes
   - participantes → Tabla completa de participantes con buscador en tiempo real
   - ponentes    → Tabla completa de ponentes con área, fecha, hora, archivo, recibo
   
   FUNCIONES:
   - Ver archivos subidos por ponentes (PDFs/DOCX de trabajos)
   - Ver recibos de pago de cada registro
   - Buscador JS que filtra cualquier columna de las tablas
   - Badges de estado: Completado (verde) / Pendiente (amarillo) / Cancelado (rojo)

   SESIÓN REQUERIDA: $_SESSION['admin_id']
   ============================================================ */
    ...
    <div class="tabla-box">
        ...
        <table id="tabla-pon">
            <thead><tr>
                ...
                <th>Área</th><th>Tipo</th><th>Fecha Propuesta</th><th>Hora</th>
                <th>Archivo</th><th>Modalidad</th>
                <th>Monto</th><th>Estado</th><th>Recibo</th><th>Fecha Registro</th>
            </tr></thead>
            <tbody>
            <?php foreach ($ponentes as $po): ?>
                <tr>
                    ...
                    <td><?= htmlspecialchars($po['titulo_trabajo']) ?></td>
                    <td><?= htmlspecialchars($po['area_tematica'] ?? '—') ?></td>
                    <td><span class="badge <?= $po['tipo_envio']==='ponencia' ? 'badge-ponen' : 'badge-memo' ?>">
                        <?= ucfirst($po['tipo_envio']) ?></span></td>
                    <td><?= htmlspecialchars($po['fecha_ponencia'] ?? '—') ?></td>
                    <td><?= htmlspecialchars($po['hora_ponencia'] ?? '—') ?></td>
                    <td>
                        <?php if ($po['archivo_ruta']): ?>
                            <a class="btn-sm" href="../<?= htmlspecialchars($po['archivo_ruta']) ?>" target="_blank">📎 Ver</a>
                        <?php else: ?> – <?php endif; ?>
                    </td>
